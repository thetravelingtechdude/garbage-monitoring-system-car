// We use a NodeMCU controller to monitor Dustbin 1
...
void setup()
{
  ...
  lcd.print(4,0,"Dustbin 1"); // use: (position X: 0-15, position Y: 0-1, "Message you want to print")
  delay(1000);
...
void loop()
{
  ...
  {
    lcd.clear();
    lcd.print(0,0,"Dustbin1 Empty %");
    lcd.print(7, 1,distance%100*10-20);
    if(distance <= 3)
    {
      led.on();
      Blynk.setProperty(V0,"color","#e80606");  // Red
    }
    else
    {
      led.on();
      Blynk.setProperty(V0,"color","#d69e04");  // Orange
    }
  }
  else if(distance <=0  || distance > 10)
  {
    lcd.print(0,1,"Dustbin1 Empty");
    led.on();
    Blynk.setProperty(V0,"color","#0c990c");  // Green
      if (distance<=0)
        goto l;
  }
  Blynk.run();
